...
namespace App\Libs\Generate;

use App\Model\Admin\AdminMenu;
use App\Libs\Generate\Field\Editor;
use App\Libs\Generate\Field\Field;
use App\Libs\Generate\Field\File;
use App\Libs\Generate\Field\Image;
use App\Libs\Generate\Field\MultiFile;
use App\Libs\Generate\Field\MultiImage;
use App\Libs\Generate\Rule\Required;
use Illuminate\Support\Facades\DB;
use App\Libs\Generate\Traits\Tools;
use App\Libs\Generate\Util\Tree;
...
class Generate
{
    ...
    public function __construct($data = [], $config = null)
    {
        ...
        $config_tmp = [
            //模版目录
            'template' => [
                'path'       => $app_path . '/Libs/Generate/Stub',
                'controller' => $app_path . '/Libs/Generate/Stub/Controller.stub',
                'model'      => $app_path . '/Libs/Generate/Stub/Model.stub',
                'validate'   => $app_path . '/Libs/Generate/Stub/Validate.stub',
                'view'       => [
                    'index'         => $app_path . '/Libs/Generate/Stub/view/index.stub',
                    'index_path'    => $app_path . '/Libs/Generate/Stub/view/index/',
                    'index_del1'    => $app_path . '/Libs/Generate/Stub/view/index/del1.stub',
                    'index_del2'    => $app_path . '/Libs/Generate/Stub/view/index/del2.stub',
                    'index_filter'  => $app_path . '/Libs/Generate/Stub/view/index/filter.stub',
                    'index_export'  => $app_path . '/Libs/Generate/Stub/view/index/export.stub',
                    'index_select1' => $app_path . '/Libs/Generate/Stub/view/index/select1.stub',
                    'index_select2' => $app_path . '/Libs/Generate/Stub/view/index/select2.stub',
                    'add'           => $app_path . '/Libs/Generate/Stub/view/add.stub',
                ],
            ],
            //生成文件目录
            'file_dir' => [
                'controller' => $app_path . '/Http/Controllers/Admin',
                'model'      => $app_path . '/Model/Common',
                'validate'   => $app_path . '/Validate/Common',
                'view'       => base_path() . '/resources/views/admin',
            ],
        ];
    ...
    {
        $table_data = DB::select('SHOW TABLES');
    ...
    {
        $result = AdminMenu::where('id', '<>', $current_id)->orderBy('sort_id', 'asc')->orderBy('id', 'asc')->get(['id','parent_id','name','sort_id'])->toArray();
    ...
    protected function createTable($data = [])
    {
        ...
        //表名
        $name = config('database.connections.mysql.prefix') . $data['table'];
    ...
    protected function createModel()
    {
        ...
        $soft_delete1   = 'use Illuminate\Database\Eloquent\SoftDeletes;';
    ...
    protected function createMenu()
    {
        ...
        try {
            ...
            if (AdminMenu::where('url', $url_prefix . '/index')->first()) {
                exception('菜单已存在');
            }
    ...
    public function getAll($table_name)
    {
        ...
        //所有字段信息
        $field_list = DB::select('SHOW FULL COLUMNS FROM `' . $table_name . '`');
        $field_list = json_decode(json_encode($field_list),true);
        //表信息
        $table_info = DB::select('SHOW TABLE STATUS LIKE ' . "'" . $table_name . "'");
